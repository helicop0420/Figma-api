...
const App = () => {
  ...
  const onChecked = (e: React.ChangeEvent<HTMLInputElement>, item: any) => {
    if(e.target.checked) {
      ...
      setContributors(contributors.filter(contributor => contributor != item))
    } else {
      setContributors([...contributors, item])
      setReviews(reviews.filter(review => review != item))
    }
  }
  ...
  return (
    <section className={styles.wrap}>
      {contributors.length === 0 && reviews.length === 0 && (
        <p className={styles.emptyText}>No contributors</p>
      )}
      <Box style={{flexGrow: 1}}>
        {contributors.length === 0 && reviews.length > 0 && (
          <Box sx={{display: 'flex', alignItems:'center', flexDirection: 'column', paddingBottom: '32px'}}>
            <p className={styles.contributed}>Everyone has contributed.</p>
            <Button className={styles.resetBtn} variant="outlined" onClick={onReset}>Reset contributors</Button>
          </Box>
        )}

        {contributors.length > 0 && (
          <>
            <h1 className={styles.headingTitle}>{contributors.length} Contributor</h1>
              ...
              <Box key={contributor.id} sx={{display: 'flex', justifyContent:'space-between', alignItems: 'center', marginBottom: '10px'}}>
                <Box sx={{display: 'flex', alignItems: 'center'}}>
                  <Checkbox
                    ...
                    sx={{marginLeft: '-9px'}}
                  />
                  <div className={styles.userAvatar} style={{backgroundColor: `${contributor.avatarColor}`}} >
                    {contributor.name[0].toUpperCase()}  
                  </div>
                  <p  className={styles.userName}>{contributor.name}</p>
                </Box>
                <div className={styles.stickyCounter} style={{backgroundColor: `${contributor.stickyColor}`}}>
                  {contributor.sticky}
                </div>
              </Box>
            ))}
              ...
              <Box key={review.id} sx={{display: 'flex', justifyContent:'space-between', alignItems: 'center', marginBottom: '10px'}}>
                <Box sx={{display: 'flex', alignItems: 'center'}}>
                  <Checkbox
                    ...
                    sx={{marginLeft: '-9px'}}
                  />
                  <div className={styles.userAvatar} style={{backgroundColor: `${review.avatarColor}`}} >
                    {review.name[0].toUpperCase()}
                  </div>
                  <p  className={styles.userName}>{review.name}</p>
                </Box>
                <div className={styles.stickyCounter} style={{backgroundColor: `${review.stickyColor}`}}>
                  {review.sticky}
                </div>
              </Box>
            ))}
      ...
      <Box sx={{display: 'flex', 
        ...
        <Button className={styles.footBtn} onClick={onReset}>Reset Contributors</Button>
        <Button className={styles.footBtn} onClick={onClear}>Clear</Button>
      </Box>
    </section>
  );
};
